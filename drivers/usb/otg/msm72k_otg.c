...
/* for leo clk reset */
#define CLKRGM_APPS_RESET_USBH      36 // orign:37, x10: 36
#define CLKRGM_APPS_RESET_USB_PHY   52 // orign:34, x10: 52
...
static int msm_otg_resume(struct msm_otg *dev)
{
	 ...
	 */
#ifdef CONFIG_MACH_HTCLEO /* this marco is ported from SE x10 */
	otg_pm_qos_update_axi(dev, 0);
#else
...
static int msm_otg_set_suspend(struct otg_transceiver *xceiv, int suspend)
{
	...
	pr_info("suspend request in state: %s\n",
			state_string(state));
...
static irqreturn_t msm_otg_irq(int irq, void *data)
{
	...
	if (!((otgsc & sts_mask) || (sts & STS_PCI))) {
		...
		pr_info("%s: STS_MASK or STS_PCI IRQ.\n", __func__);
		ret = IRQ_NONE;
		goto out;
	}
...
static void phy_clk_reset(struct msm_otg *dev)
{
        ...
        int assert = PCOM_CLK_REGIME_SEC_RESET_ASSERT;

        if (dev->pdata->phy_reset_sig_inverted)
                assert = PCOM_CLK_REGIME_SEC_RESET_DEASSERT;


        ret = msm_proc_comm(assert,
                        &usb_phy_id, NULL);
        if (ret) {
        ...
        msleep(1);
        ret = msm_proc_comm( (assert == PCOM_CLK_REGIME_SEC_RESET_DEASSERT) ? PCOM_CLK_REGIME_SEC_RESET_ASSERT : assert,
                        &usb_phy_id, NULL);
        if (ret) {
                printk(KERN_INFO "%s: Cannot assert (%d)\n", __func__, ret);
                return;
...
static void msm_otg_apps_reset_link(int reset)
{
        int ret;
...
static int msm_otg_phy_reset(struct msm_otg *dev)
{
	...
	msm_otg_apps_reset_link(1);
	phy_clk_reset(dev);
	msm_otg_apps_reset_link(0);
...
reset_link:
	...
	if ((xceiv->gadget && xceiv->gadget->is_a_peripheral) ||
		...
		pr_info("%s: reset usb to device mode\n", __func__);
		mode = USBMODE_SDIS | USBMODE_DEVICE;
	} else {

		pr_info("%s: reset usb to host mode\n", __func__);
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	if (dev->pdata->core_clk) {
		...
		clk_enable(dev->hs_cclk);
		pr_info("msm_otg: hs core clk saved\n");
	}
	...
	if (!dev->pdata->phy_reset) {
		...
		pr_info("msm_otg: usb phy clk saved\n");
	}
...
#ifdef CONFIG_MACH_HTCLEO /* this marco ported from SE x10 */
	otg_pm_qos_update_axi(dev, 0);
#else
	otg_pm_qos_update_axi(dev, 1);
#endif
